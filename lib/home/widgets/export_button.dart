...
class ExportButton extends StatelessWidget {
  ...
  void _onPressed(BuildContext context) async {
    ...
    if (user != null) {
      // Convert the theme data to JSON
      final themeJson = _themeToJson(theme);

      // Get a Firestore reference and store the theme data
      final CollectionReference themesCollection = FirebaseFirestore.instance.collection('themes');
      final exportableJson = ThemeEncoder.encodeThemeData(theme);
      final themeStr = prettyJson(themeJson);

      await themesCollection.doc("Sharanya").set(exportableJson).then((_) {
        print('Theme saved to Firestore');
      }).catchError((error) {
        print('Failed to save theme: $error');
      });

      // Notify the cubit that the theme has been exported
...
// Helper function to convert ThemeData to JSON
  Map<String, dynamic> _themeToJson(ThemeData theme) {
    return {
      'primaryColor': theme.primaryColor.value.toString(),
      'accentColor': theme.colorScheme.secondary?.value.toString(),
